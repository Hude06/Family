...
OpenLayers.Map = OpenLayers.Class({
  ...
  setOptions: function (options) {
    ...
    updatePxExtent &&
      this.moveTo(this.getCachedCenter(), this.zoom, {
  ...
  panTo: function (lonlat) {
    if (
      this.panTween &&
      this.getExtent().scale(this.panRatio).containsLonLat(lonlat)
    ) {
      var center = this.getCachedCenter();
  ...
  zoomTo: function (zoom, xy) {
    ...
    if (map.isValidZoomLevel(zoom)) {
      ...
      if (map.zoomTween) {
        ...
        if (
          map.zoomTween.playing &&
          map.zoomTween.duration < 3 * map.zoomDuration
        ) {
          ...
          map.zoomTween.start(start, end, map.zoomDuration, {
            ...
            callbacks: {
              eachStep: function (data) {
                ...
                map.applyTransform(
                  containerOrigin.x + dx,
                  containerOrigin.y + dy,
                  scale,
                );
              },
              done: function (data) {
  ...
  applyTransform: function (x, y, scale) {
    ...
    if (transform === undefined) {
      ...
      if (transform) {
        ...
        var computedStyle = OpenLayers.Element.getStyle(
          this.viewPortDiv,
    ...
    if (
      transform !== null &&
      (template[0] === "translate3d(" || needTransform === true)
    ) {
