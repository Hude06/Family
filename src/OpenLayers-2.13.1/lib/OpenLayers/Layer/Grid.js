...
OpenLayers.Layer.Grid = OpenLayers.Class(OpenLayers.Layer.HTTPRequest, {
  ...
  addOptions: function (newOptions, reinitialize) {
    var singleTileChanged =
      newOptions.singleTile !== undefined &&
  ...
  calculateGridLayout: function (bounds, origin, resolution) {
    ...
    var tilerow =
      Math[~rowSign ? "floor" : "ceil"](offsetlat / tilelat) -
      this.buffer * rowSign;
  ...
  initGriddedTiles: function (bounds) {
    ...
    do {
      ...
      do {
        ...
        tileBounds.right <= bounds.right + tilelon * this.buffer ||
        colidx < minCols
      );

      rowidx += 1;
    } while (
      tileBounds.bottom >= bounds.bottom - tilelat * this.buffer ||
      rowidx < minRows
    );
