...
OpenLayers.Control.OverviewMap = OpenLayers.Class(OpenLayers.Control, {
  ...
  draw: function () {
    ...
    if (!this.outsideViewport) {
      ...
      this.maximizeDiv.className =
        this.displayClass + "MaximizeButton olButton";
      ...
      this.minimizeDiv.className =
        this.displayClass + "MinimizeButton olButton";
  ...
  mapDivClick: function (evt) {
    ...
    this.setRectPxBounds(
      new OpenLayers.Bounds(newLeft, newTop + height, newLeft + width, newTop),
    );
  ...
  updateOverview: function () {
    ...
    this.ovmap.setCenter(
      center,
      this.ovmap.getZoomForResolution(targetRes * this.resolutionFactor),
    );
    this.updateRectToMap();
  },
  ...
  createMap: function () {
    ...
    var options = OpenLayers.Util.extend(
      { controls: [], maxResolution: "auto", fallThrough: false },
      this.mapOptions,
    );
    ...
    this.wComp =
      ...
      parseInt(
        OpenLayers.Element.getStyle(this.extentRectangle, "border-right-width"),
      );
    this.wComp = this.wComp ? this.wComp : 2;
    this.hComp =
      parseInt(
        OpenLayers.Element.getStyle(this.extentRectangle, "border-top-width"),
      ) +
      parseInt(
        OpenLayers.Element.getStyle(
    ...
    this.rectEvents = new OpenLayers.Events(
      this,
      this.extentRectangle,
      null,
      true,
    ...
    if (this.ovmap.getProjection() != this.map.getProjection()) {
      var sourceUnits =
        this.map.getProjectionObject().getUnits() ||
        this.map.units ||
      ...
      this.resolutionFactor =
        sourceUnits && targetUnits
          ? OpenLayers.INCHES_PER_UNIT[sourceUnits] /
            OpenLayers.INCHES_PER_UNIT[targetUnits]
          : 1;
    }
  },
  ...
  setRectPxBounds: function (pxBounds) {
    ...
    var bottom = Math.min(
      pxBounds.top + Math.abs(pxBounds.getHeight()),
      this.ovmap.size.h - this.hComp,
    );
    var right = Math.min(
      pxBounds.left + pxBounds.getWidth(),
      this.ovmap.size.w - this.wComp,
    );
    var width = Math.max(right - left, 0);
    var height = Math.max(bottom - top, 0);
    if (width < this.minRectSize || height < this.minRectSize) {
      this.extentRectangle.className =
        this.displayClass + this.minRectDisplayClass;
  ...
  getMapBoundsFromRectBounds: function (pxBounds) {
    ...
    return new OpenLayers.Bounds(
      leftBottomLonLat.lon,
      leftBottomLonLat.lat,
      rightTopLonLat.lon,
      rightTopLonLat.lat,
    );
  },
