...
OpenLayers.Layer.Zoomify = OpenLayers.Class(OpenLayers.Layer.Grid, {
  ...
  initializeZoomify: function (size) {
    ...
    while (
      imageSize.w > this.standardTileSize ||
  ...
  getURL: function (bounds) {
    ...
    var x = Math.round(
      (bounds.left - this.tileOrigin.lon) / (res * this.tileSize.w),
    );
    var y = Math.round(
      (this.tileOrigin.lat - bounds.top) / (res * this.tileSize.h),
    );
    var z = this.getZoomForResolution(res);

    var tileIndex =
      x + y * this.tierSizeInTiles[z].w + this.tileCountUpToTier[z];
    var path =
      "TileGroup" +
      Math.floor(tileIndex / 256) +
      "/" +
      z +
      "-" +
      x +
