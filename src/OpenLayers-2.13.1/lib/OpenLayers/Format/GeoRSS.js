...
OpenLayers.Format.GeoRSS = OpenLayers.Class(OpenLayers.Format.XML, {
  ...
  createGeometryFromItem: function (item) {
    ...
    if (point.length > 0 || (lat.length > 0 && lon.length > 0)) {
      var location;
      if (point.length > 0) {
        location = OpenLayers.String.trim(point[0].firstChild.nodeValue).split(
          /\s+/,
        );
        if (location.length != 2) {
          location = OpenLayers.String.trim(
            point[0].firstChild.nodeValue,
          ).split(/\s*,\s*/);
        }
      } else {
        location = [
          parseFloat(lat[0].firstChild.nodeValue),
      ...
      var coords = OpenLayers.String.trim(this.getChildValue(line[0])).split(
        /\s+/,
      );
      var components = [];
      var point;
      ...
      var coords = OpenLayers.String.trim(this.getChildValue(polygon[0])).split(
        /\s+/,
      );
      var components = [];
      var point;
      ...
      var coords = OpenLayers.String.trim(box[0].firstChild.nodeValue).split(
        /\s+/,
      );
      var components = [];
      var point;
  ...
  createFeatureFromItem: function (item) {
    ...
    if (!link) {
      try {
        link = this.getElementsByTagNameNS(item, "*", "link")[0].getAttribute(
          "href",
        );
      } catch (e) {
        link = null;
