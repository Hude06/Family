...
OpenLayers.Format.GML = OpenLayers.Class(OpenLayers.Format.XML, {
  ...
  parseFeature: function (node) {
    ...
    for (i = 0; i < boxNodes.length; ++i) {
      ...
      var parentName =
        parentNode.localName || parentNode.nodeName.split(":").pop();
  ...
  parseGeometry: {
      ...
      if (coords.length == 0) {
        ...
        if (nodeList.length > 0) {
          ...
          for (var i = 0; i < pointList.length; ++i) {
            ...
            if (this.xy) {
              points.push(
                new OpenLayers.Geometry.Point(coords[0], coords[1], coords[2]),
              );
            } else {
              points.push(
                new OpenLayers.Geometry.Point(coords[1], coords[0], coords[2]),
              );
            }
          }
        }
    ...
    envelope: function (node) {
      ...
      if (lpoint.length > 0) {
        ...
        if (this.xy) {
          var lowerPoint = new OpenLayers.Geometry.Point(
            coords[0],
            coords[1],
            coords[2],
          );
      ...
      if (upoint.length > 0) {
        ...
        if (this.xy) {
          var upperPoint = new OpenLayers.Geometry.Point(
            coords[0],
            coords[1],
            coords[2],
          );
      ...
      if (lowerPoint && upperPoint) {
        components.push(
          new OpenLayers.Geometry.Point(lowerPoint.x, lowerPoint.y),
        );
        components.push(
          new OpenLayers.Geometry.Point(upperPoint.x, lowerPoint.y),
        );
        components.push(
          new OpenLayers.Geometry.Point(upperPoint.x, upperPoint.y),
        );
        components.push(
          new OpenLayers.Geometry.Point(lowerPoint.x, upperPoint.y),
        );
        components.push(
          new OpenLayers.Geometry.Point(lowerPoint.x, lowerPoint.y),
        );
  ...
  createFeatureXML: function (feature) {
    ...
    var geomContainer = this.createElementNS(
      this.featureNS,
    ...
    var featureContainer = this.createElementNS(
      this.featureNS,
