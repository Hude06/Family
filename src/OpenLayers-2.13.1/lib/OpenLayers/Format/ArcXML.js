...
OpenLayers.Format.ArcXML = OpenLayers.Class(OpenLayers.Format.XML, {
  ...
  write: function (request) {
    ...
    if (request.get_image != null) {
      ...
      if (
        props.imagesize.height != props.imagesize.printheight ||
      ...
      if (props.layerlist != null && props.layerlist.length > 0) {
        ...
        for (var ld = 0; ld < props.layerlist.length; ld++) {
          ...
          if (typeof props.layerlist[ld].query == "object") {
            ...
            var queryElem = null;
            if (
              typeof query.spatialfilter == "boolean" &&
              query.spatialfilter
            ) {
            ...
            if (
              typeof query.featurelimit == "number" &&
              query.featurelimit < 2000
            ) {
              queryElem.setAttribute("featurelimit", query.featurelimit);
            }
            if (
              typeof query.subfields == "string" &&
              query.subfields != "#ALL#"
            ) {
              queryElem.setAttribute("subfields", query.subfields);
            }
            if (
              typeof query.joinexpression == "string" &&
              query.joinexpression.length > 0
            ) {
              queryElem.setAttribute("joinexpression", query.joinexpression);
            }
            if (
              typeof query.jointables == "string" &&
              query.jointables.length > 0
            ) {
      ...
      if (
        request.get_feature.layer != null &&
        request.get_feature.layer.length > 0
      ) {
  ...
  addRenderer: function (topRelem, renderer) {
    if (OpenLayers.Util.isArray(renderer)) {
      ...
      var renderElem = this.createElementNS(
        "",
        renderer.type.toUpperCase() + "RENDERER",
      );
  ...
  addScaleDependentRenderer: function (renderElem, renderer) {
    if (
      typeof renderer.lower == "string" ||
      typeof renderer.lower == "number"
    ) {
      renderElem.setAttribute("lower", renderer.lower);
    }
    if (
      typeof renderer.upper == "string" ||
      typeof renderer.upper == "number"
    ) {
    ...
    if (errorNode != null && errorNode.length > 0) {
      ...
      if (rtype == "IMAGE") {
        ...
        if (response.features.featurecount > 0) {
          ...
          response.features.envelope = this.parseAttributes(
            envelope[0],
            typeof 0,
          );
  ...
  parseAttributes: function (node, type) {
    ...
    for (var attr = 0; attr < node.attributes.length; attr++) {
      if (type == "number") {
        attributes[node.attributes[attr].nodeName] = parseFloat(
          node.attributes[attr].nodeValue,
        );
      } else {
        attributes[node.attributes[attr].nodeName] =
          node.attributes[attr].nodeValue;
      }
    }
    return attributes;
  ...
  parsePointGeometry: function (node) {
    ...
    if (coords.length > 0) {
      ...
      for (var cn = 0; cn < coordArr.length; cn++) {
        ...
        ringPoints.push(
          new OpenLayers.Geometry.Point(coordItems[0], coordItems[1]),
        );
      }
      coords = null;
    } else {
