...
OpenLayers.Format.GPX = OpenLayers.Class(OpenLayers.Format.XML, {
  ...
  read: function (doc) {
    ...
    if (this.extractTracks) {
      ...
      for (var i = 0, len = tracks.length; i < len; i++) {
        ...
        var segs = this.getElementsByTagNameNS(
          tracks[i],
          tracks[i].namespaceURI,
          "trkseg",
        );
    ...
    if (this.extractWaypoints) {
      ...
      for (var l = 0, len = waypoints.length; l < len; l++) {
        ...
        var wpt = new OpenLayers.Geometry.Point(
          waypoints[l].getAttribute("lon"),
          waypoints[l].getAttribute("lat"),
        );
  ...
  extractSegment: function (segment, segmentType) {
    var points = this.getElementsByTagNameNS(
      segment,
      segment.namespaceURI,
      segmentType,
    );
    var point_features = [];
  ...
  appendAttributesNode: function (node, feature) {
    ...
    name.appendChild(
      this.createTextNode(feature.attributes.name || feature.id),
    );
    node.appendChild(name);
    var desc = this.createElementNS(this.namespaces.gpx, "desc");
    desc.appendChild(
      this.createTextNode(feature.attributes.description || this.defaultDesc),
    );
