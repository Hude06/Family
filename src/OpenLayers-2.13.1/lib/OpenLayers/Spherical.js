...
OpenLayers.Spherical.computeDistanceBetween = function (from, to, radius) {
  ...
  var a =
    ...
    sinHalfDeltaLon *
      sinHalfDeltaLon *
      Math.cos((Math.PI * from.lat) / 180) *
      Math.cos((Math.PI * to.lat) / 180);
...
OpenLayers.Spherical.computeHeading = function (from, to) {
  var y =
    Math.sin((Math.PI * (from.lon - to.lon)) / 180) *
    Math.cos((Math.PI * to.lat) / 180);
  var x =
    Math.cos((Math.PI * from.lat) / 180) * Math.sin((Math.PI * to.lat) / 180) -
    Math.sin((Math.PI * from.lat) / 180) *
      Math.cos((Math.PI * to.lat) / 180) *
      Math.cos((Math.PI * (from.lon - to.lon)) / 180);
