  ...
  buildContentNode: function (content) {
    ...
    if (content.src) {
      ...
      if (content.type == "text" || content.type == null) {
        ...
        // MUST be a valid Base64 encoding
        node.appendChild(this.createTextNode(content.value));
      }
    }
    return node;
  ...
  buildEntryNode: function (feature) {
    ...
    if (atomAttrib.authors) {
      ...
      for (var i = 0, ii = authors.length; i < ii; i++) {
        entryNode.appendChild(
          this.buildPersonConstructNode("author", authors[i]),
        );
      }
    }
    ...
    if (atomAttrib.contributors) {
      var contributors = OpenLayers.Util.isArray(atomAttrib.contributors)
        ? atomAttrib.contributors
        : [atomAttrib.contributors];
      for (var i = 0, ii = contributors.length; i < ii; i++) {
        entryNode.appendChild(
          this.buildPersonConstructNode("contributor", contributors[i]),
        );
      }
    }
  ...
  parseLocations: function (node) {
    ...
    if (point && point.length > 0) {
      for (var i = 0, ii = point.length; i < ii; i++) {
        var xy = OpenLayers.String.trim(point[i].firstChild.nodeValue).split(
          /\s+/,
        );
        if (xy.length != 2) {
          xy = OpenLayers.String.trim(point[i].firstChild.nodeValue).split(
            /\s*,\s*/,
          );
        }
    ...
    if (line && line.length > 0) {
      ...
      for (var i = 0, ii = line.length; i < ii; i++) {
        coords = OpenLayers.String.trim(line[i].firstChild.nodeValue).split(
          /\s+/,
        );
        points = [];
    ...
    if (polygon && polygon.length > 0) {
      ...
      for (var i = 0, ii = polygon.length; i < ii; i++) {
        coords = OpenLayers.String.trim(polygon[i].firstChild.nodeValue).split(
          /\s+/,
        );
        points = [];
        for (var j = 0, jj = coords.length; j < jj; j += 2) {
