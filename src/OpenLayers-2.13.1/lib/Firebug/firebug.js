  ...
  (function () {
    ...
    function layout() {
      ...
      commandLine.style.top =
        consoleFrame.offsetHeight - commandLine.offsetHeight + "px";
    }

    function logRow(message, className, handler) {
      if (consoleBody) writeMessage(message, className, handler);
      else {
    ...
    function appendObject(object, html) {
      try {
        if (object == undefined) appendNull("undefined", html);
    ...
    function onKeyDown(event) {
      if (event.keyCode == 123) toggleConsole();
        ...
        (event.keyCode == 108 || event.keyCode == 76) &&
        event.shiftKey &&
    ...
    function onCommandLineKeyDown(event) {
      if (event.keyCode == 13) evalCommandLine();
