...
function init() {
  ...
  map.addControl(
    new OpenLayers.Control.SelectFeature(waterBodies, {
      hover: true,
      autoActivate: true,
    }),
  );
...
function createLayers() {
  ...
  for (var i = 0, ii = layerData.length; i < ii; ++i) {
    layers.push(
      new OpenLayers.Layer.Vector(layerData[i].name, {
...
function setLayerStyles() {
  ...
  waterBodies.styleMap.styles.select =
    sld.namedLayers["WaterBodies"].userStyles[1];
}
...
function setStyle(index) {
  waterBodies.styleMap.styles["default"] =
    sld.namedLayers["WaterBodies"].userStyles[index];
