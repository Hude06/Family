...
OpenLayers.Format.WMC.v1 = OpenLayers.Class(OpenLayers.Format.XML, {
  ...
  write_wmc_General: function (context) {
    ...
    if (context.size) {
      node.appendChild(
        this.createElementDefaultNS("Window", null, {
    ...
    if (context["abstract"]) {
      node.appendChild(
        this.createElementDefaultNS("Abstract", context["abstract"]),
      );
    }
    ...
    if (context.logo) {
      node.appendChild(
        this.write_wmc_URLType("LogoURL", context.logo.href, context.logo),
      );
    }
    ...
    if (context.descriptionURL) {
      node.appendChild(
        this.write_wmc_URLType("DescriptionURL", context.descriptionURL),
      );
    }
    ...
    if (context.contactInformation) {
      node.appendChild(
        this.write_wmc_ContactInformation(context.contactInformation),
      );
    }
  ...
  write_wmc_ContactInformation: function (contact) {
    ...
    if (contact.personPrimary) {
      node.appendChild(
        this.write_wmc_ContactPersonPrimary(contact.personPrimary),
      );
    }
    if (contact.position) {
      node.appendChild(
        this.createElementDefaultNS("ContactPosition", contact.position),
      );
    }
    ...
    if (contact.phone) {
      node.appendChild(
        this.createElementDefaultNS("ContactVoiceTelephone", contact.phone),
      );
    }
    if (contact.fax) {
      node.appendChild(
        this.createElementDefaultNS("ContactFacsimileTelephone", contact.fax),
      );
    }
    if (contact.email) {
      node.appendChild(
  ...
  write_wmc_ContactPersonPrimary: function (personPrimary) {
    ...
    if (personPrimary.person) {
      node.appendChild(
        this.createElementDefaultNS("ContactPerson", personPrimary.person),
      );
    }
  ...
  write_wmc_ContactAddress: function (contactAddress) {
    ...
    if (contactAddress.type) {
      node.appendChild(
        this.createElementDefaultNS("AddressType", contactAddress.type),
      );
    }
    if (contactAddress.address) {
      node.appendChild(
        this.createElementDefaultNS("Address", contactAddress.address),
      );
    }
    if (contactAddress.city) {
      node.appendChild(
        this.createElementDefaultNS("City", contactAddress.city),
      );
    }
    ...
    if (contactAddress.postcode) {
      node.appendChild(
        this.createElementDefaultNS("PostCode", contactAddress.postcode),
      );
    }
    if (contactAddress.country) {
      node.appendChild(
        this.createElementDefaultNS("Country", contactAddress.country),
      );
    }
    return node;
  },
  ...
  write_wmc_Layer: function (context) {
    ...
    if (context.metadataURL) {
      node.appendChild(
        this.write_wmc_URLType("MetadataURL", context.metadataURL),
      );
    }
  ...
  write_wmc_DimensionList: function (context) {
    ...
    for (var dim in context.dimensions) {
      ...
      node.appendChild(
        this.createElementDefaultNS("Dimension", values, attributes),
      );
    }
    return node;
  },
    ...
    for (var i = 0, len = context.formats.length; i < len; i++) {
      ...
      node.appendChild(
        this.createElementDefaultNS(
          "Format",
          format.value,
          format.current && format.current == true ? { current: "1" } : null,
        ),
      );
    }
    ...
    if (styles && OpenLayers.Util.isArray(styles)) {
      ...
      for (var i = 0, len = styles.length; i < len; i++) {
        ...
        var style = this.createElementDefaultNS(
          "Style",
          null,
          s.current && s.current == true ? { current: "1" } : null,
        );
        if (s.href) {
          // [1]
          ...
          if (s.legend) {
            sld.appendChild(
              this.write_wmc_URLType("LegendURL", s.legend.href, s.legend),
            );
          }
