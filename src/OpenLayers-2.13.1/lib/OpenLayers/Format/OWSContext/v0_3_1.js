    ...
    owc: {
      ...
      Layer: function (node, obj) {
        var layerContext = {
          ...
          opacity:
            node.getAttribute("opacity") != null
              ? parseFloat(node.getAttribute("opacity"))
              : null,
      ...
      InlineGeometry: function (node, obj) {
        ...
        if (el && el.firstChild) {
          ...
          this.featureType =
            featurenode.localName || featurenode.nodeName.split(":").pop();
          this.readChildNodes(node, obj);
        }
      },
      Server: function (node, obj) {
        ...
        if (
          (!obj.service && !obj.version) ||
          obj.service != OpenLayers.Format.Context.serviceTypes.WMS
        ) {
  ...
  writers: {
    owc: {
      OWSContext: function (options) {
        var node = this.createElementNSPlus("OWSContext", {
          attributes: {
            ...
            id:
              options.id ||
              OpenLayers.Util.createUniqueID("OpenLayers_OWSContext_"),
          },
        });
      ...
      General: function (options) {
        ...
        this.writeNode(
          "ows:Title",
          options.title || "OpenLayers OWSContext",
          node,
        );
        return node;
      },
      ResourceList: function (options) {
        ...
        for (var i = 0, len = options.layers.length; i < len; i++) {
          ...
          var decomposedPath = this.decomposeNestingPath(
            layer.metadata.nestingPath,
          );
          this.writeNode(
            "_Layer",
            { layer: layer, subPaths: decomposedPath },
            node,
          );
        }
        return node;
      ...
      _WFS: function (layer) {
        var node = this.createElementNSPlus("Layer", {
          attributes: {
            name:
              layer.protocol.featurePrefix + ":" + layer.protocol.featureType,
            hidden: layer.visibility ? "0" : "1",
          },
        });
        this.writeNode("ows:Title", layer.name, node);
