...
OpenLayers.Control.Graticule = OpenLayers.Class(OpenLayers.Control, {
  ...
  update: function () {
    ...
    do {
      ...
      mapXY = OpenLayers.Projection.transform(
        newPoint.clone(),
        llProj,
        mapProj,
      );
    ...
    for (var i = 0; i < centerLatPoints.length; ++i) {
      ...
      var latStart = Math.max(
        centerLonPoints[centerLonPoints.length - 1].y,
        -90,
      );
      ...
      if (this.labelled) {
        ...
        var labelPos = new OpenLayers.Geometry.Point(
          labelPoint.x,
          mapBounds.bottom,
        );
        var labelAttrs = {
          value: lon,
          label: this.labelled
            ? OpenLayers.Util.getFormattedLonLat(lon, "lon", this.labelFormat)
            : "",
          labelAlign: "cb",
          xOffset: 0,
          yOffset: 2,
        };
        this.gratLayer.addFeatures(
          new OpenLayers.Feature.Vector(labelPos, labelAttrs),
        );
      }
    ...
    for (var j = 0; j < centerLonPoints.length; ++j) {
      ...
      if (lat < -90 || lat > 90) {
        //latitudes only valid between -90 and 90
        continue;
      }
      var pointList = [];
      ...
      if (this.labelled) {
        ...
        var labelAttrs = {
          value: lat,
          label: this.labelled
            ? OpenLayers.Util.getFormattedLonLat(lat, "lat", this.labelFormat)
            : "",
          labelAlign: "rb",
          xOffset: -2,
          yOffset: 2,
