...
function init() {
  ...
  function updateStatus(evt) {
    if (window.localStorage) {
      ...
      status.innerHTML =
        "Local storage not supported. Try a different browser.";
    }
  ...
  function seed() {
    ...
    var extentWidth =
      seeding.extent.getWidth() / map.getResolutionForZoom(nextZoom);
    // adjust the layer's buffer size so we don't have to pan
    layer.buffer = Math.ceil(
      (extentWidth / tileWidth - map.getSize().w / tileWidth) / 2,
    );
    map.zoomIn();
